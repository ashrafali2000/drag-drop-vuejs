<!-- <template>
  <div class="">
    ...
    <div
      class="MatcDownloadDialogRender"
      data-dojo-attach-point="renderCntr"
    ></div>
  </div>
</template>
  ...
  methods: {
    ...
    async render(model) {
      ...
      for (let id in model.screens) {
        ...
        let wrapper = this.db
          .div(
            "MatcToolbarScreenListPreviewWrapper MatcCreateBtnElement MatcToolbarDropDownButtonItem"
          )
    ...
    onDataUrl(screen, e) {
      ...
      if (wrapper) {
        ...
        this.tempOwn(
          on(wrapper, "click", lang.hitch(this, "download", screen))
        );
      }
...
export default {
  ...
  data() {
    return {
      width: 150,
      height: 150,
      blobs: {}, // Track blobs for each screen
  ...
  methods: {
    ...
    onDataUrl(screen, e) {
      ...
      if (wrapper) {
        ...
        const img = this.db.img().build(wrapper);
        img.src = e.target.result;
        this.images.push({ id: screen.id, src: e.target.result }); // Store image data
        this.db.span("", screen.name).build(wrapper);
        this.tempOwn(
          on(wrapper, "click", lang.hitch(this, "download", screen))
        );
    ...
    getImageData() {
      if (this.images.length > 0) {
        return this.images[0].src; // Return the first image's base64 string
      } else {
