<!-- <template>
  <div class="MatchImportDialog MatchExportDialog MatcDialog">
    <div class="MatcToolbarTabs MatcToolbarTabsBig">
      <a
        @click="showImageExport()"
        :class="{ MatcToolbarTabActive: tab === 'images' }"
        >{{ getNLS("dialog.export.tab-images") }}</a
      >
      <a
        @click="tab = 'github'"
        v-if="hasGit"
        :class="{ MatcToolbarTabActive: tab === 'github' }"
        >{{ getNLS("dialog.export.tab-github") }}</a
      >
      <a
        @click="tab = 'zip'"
        :class="{ MatcToolbarTabActive: tab === 'zip' }"
        >{{ getNLS("dialog.export.tab-zip") }}</a
      >
      <a
        @click="showComments()"
        :class="{ MatcToolbarTabActive: tab === 'comments' }"
        >{{ getNLS("dialog.export.tab-comments") }}</a
      >
    </div>
    <div v-if="isPublic">
    ...
    <div v-else class="MatcExportDialogCntr">
      <div v-show="tab === 'images'" ref="imageCntr"></div>
    ...
    <div class="MatcButtonBar MatcMarginTop">
      <a
        ...
        >{{ getNLS("btn.export") }}</a
      >
      <a class="MatcButton MatcButtonPrimary" @click.stop="onCancel">{{
        getNLS("btn.close")
      }}</a>
      <a
        class="MatcButton MatcButtonPrimary"
  ...
  methods: {
    ...
    async showComments() {
      ...
      if (this.commentService) {
        this.comments = await this.commentService.find(
          this.model.id,
          "ScreenComment"
        );
      }
...
<!-- <template>
  <div class="MatchImportDialog MatchExportDialog MatcDialog">
    ...
    <div class="MatcButtonBar MatcMarginTop">
      ...
      <a class="MatcButton MatcButtonPrimary" @click.stop="onShare">{{
        getNLS("btn.share")
      }}</a>
      <input v-model="shareInput1" placeholder="from: email" />
      <input v-model="shareInput2" placeholder="to: email" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onShare() {
      console.log("Share Input 1:", this.shareInput1);
      console.log("Share Input 2:", this.shareInput2);
...
export default {
  ...
  methods: {
    ...
    async onShare() {
      ...
      const imageData = this.downloader.getImageData(); 
  
      const data = {
        from: this.shareInput1,
        to: this.shareInput2,
        image: imageData,
      };
...
<template>
  <div class="MatchImportDialog MatchExportDialog MatcDialog">
    ...
    <div class="MatcError">{{ errorMSG }}</div>
...
export default {
  ...
  methods: {
    ...
    async onShare() {
      const imageData = this.downloader.getImageData(); // Retrieve image data
