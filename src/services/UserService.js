...
class UserService extends AbstractService {
  ...
  async signup(data) {
    return this._post(`${API_BASE_URL}/rest/user`, data);
  }

  async login(data) {
    let user = await this._post(`${API_BASE_URL}/rest/login/`, data);
    if (!user.errors) {
      this.setUser(user);
    }
    return user;
  ...
  save(userID, data) {
    return this._post(`${API_BASE_URL}/rest/user/${userID}.json`, data);
  }
  ...
  logout() {
    ...
    Cookies.remove("quxUserLoggedIn", { path: "/" });
    Cookies.remove("quxUserLoggedIn", { path: "/", domain: "quant-ux.com" });
    return this._delete(`${API_BASE_URL}/rest/login/`);
  }

  reset(email) {
    return this._post(`${API_BASE_URL}/rest/user/password/request`, {
      email: email,
    });
  }
  ...
  reset2(email, password, token) {
    ...
    return this._post(`${API_BASE_URL}/rest/user/password/set`, data);
  }

  retire() {
    this.logger.info("retire()", "enter > Oh oh");
    return this._get(`${API_BASE_URL}/rest/retire`);
  }

  async loadById(id) {
    return await this._get(`${API_BASE_URL}/rest/user/${id}.json`);
  }

  getNotications() {
    return this._get(`${API_BASE_URL}/rest/notifications.json`);
  }

  setLastNotication() {
    return this._post(`${API_BASE_URL}/rest/user/notification/last.json`);
  }

  getLastNotication() {
    return this._get(`${API_BASE_URL}/rest/user/notification/last.json`);
  }

  deleteImage(user) {
    return this._delete(
      `${API_BASE_URL}/rest/user/${user.id}/images/${user.image}`
    );
  }
}
