<template>
  <div
    :class="[
      'MatcPublic MatcTestPage',
      { MatcWindows: hasWindows },
      { MatcTestPageResponsive: isResponsive },
    ]"
  >
    <Splash
      :hash="hash"
    ...
    <div class="MatcTestCntr">
      <div
        class="MatcTestTaskCntr MatcTestContent"
        ref="tastCntr"
        v-if="showTasks"
      >
        <div class="MatcStudioLogo">
          <img src="/logo.webp" class="MatcStudioLogo" ref="logo" />
          <span class="MatcCollapseViewMinHidden">Bawdicsoft</span>
        </div>
        ...
        <div class="MatcTestTaskList MatcScrollContainer" ref="taskList">
          ...
          />
          <div
            class="MatcTestTaskSuccess MatcTestTask"
      ...
      <div class="MatcTest" ref="responsiveScroll">
        <div class="MatcTestSimulatorWrapper" ref="cntr"></div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getUserTasks() {
      ...
      if (this.settings.tasks && this.settings.tasks) {
        for (let i = 0; i < this.settings.tasks.length; i++) {
          ...
          if (
            task.description &&
            task.description != "Enter a description here"
          ) {
            tasks.push(task);
          }
        }
    ...
    async setModel(model) {
      ...
      if (model) {
        this.model = model;
        let test = await Services.getModelService().findTestByHash(
          this.model,
          this.hash
        );
    ...
    showQRDialog(e) {
      ...
      const dialog = this.db.div("MatchTestQRDialog MatcPadding").build();
    ...
    renderTest() {
      ...
      if (this.model.type != "desktop") {
        let btn = this.db
          .div("MatcTestQRButton  MatcAnimated MatcFadeOut")
          .build(this.domNode);
    ...
    renderResponsiceSimulator(cntr, screenPos) {
      ...
      const hasSimpleBar = ScrollUtil.addScrollIfNeeded(
        container,
        this.forceSimpleBar
      );
